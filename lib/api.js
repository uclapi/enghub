...
export const addUserToRoomWhitelist = async (roomId, email) =>
  request(`/api/rooms/user-whitelist/${roomId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email }),
  });

export const deleteUserFromRoomWhitelist = async (roomId, email) =>
  request(`/api/rooms/user-whitelist/${roomId}`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email }),
